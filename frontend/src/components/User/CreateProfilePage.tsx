...
export const CreateProfilePage: React.FC<CreateProfilePageProps> = ({
    ...
    // Redirect to edit profile if one already exists
    useEffect(() => {
        if (user?.profile_id) {
    ...
    useEffect(() => {
        if (formData.city && CITY_AREAS[formData.city]) {
            setAvailableAreas(CITY_AREAS[formData.city]);
...
// Extracted IconOption Component for clarity and syntax fix
const IconOption = ({
    label,
    value,
    currentValue,
